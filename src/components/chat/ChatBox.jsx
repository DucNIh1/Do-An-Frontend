...
// export default function ChatBox({ user, conversationId = null, onClose }) {
...
//     useInfiniteQuery({
//       queryKey: ["messages", conversationId || user?.userId],
...
//           />
//           <p className="font-medium ">{user?.name || "Group chat"}</p>
//         </div>
//         <button
//           onClick={onClose}
...
export default function ChatBox() {
  ...
  const { selectedConversation, closeConversation } = useChat();
  const { conversationId, receiver } = selectedConversation || {};
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useInfiniteQuery({
      queryKey: ["messages", conversationId || receiverId],
      queryFn: ({ pageParam = null }) =>
        getMessagesAPI({
          conversationId,
          senderId: currentUser.id,
  ...
  const queryKey = ["messages", conversationId || receiverId];
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (payload) => {
      const { conversationId: convId, message } = payload;
      ...
      if (
        ...
        receiver.conversationId = convId;
      }

      if (
        convId === (conversationId || receiver.conversationId) &&
        message.sender.id !== currentUser.id
      ) {
        queryClient.setQueryData(
          ["messages", conversationId || receiverId],
          (oldData) => {
            if (!oldData) return oldData;
    ...
    socket.on("newMessage", handleNewMessage);
    return () => socket.off("newMessage", handleNewMessage);
  }, [socket, conversationId, receiverId, currentUser, queryClient, receiver]);
      ...
      <div className="flex items-center justify-between p-2 border-b bg-gray-100">
        <div className="flex items-center gap-2">
          <img
            ...
            className="w-8 h-8 rounded-full"
          />
          <p className="font-medium">{receiver?.name || "Group chat"}</p>
        </div>
        <button
